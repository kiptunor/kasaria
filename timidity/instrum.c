...
static int32 convert_envelope_rate(Timid *tm, uint8 rate)
{
    ...
    /* 15.15 fixed point. */
    return (((r * 44100) / tm->play_mode.rate) * tm->control_ratio) << ((tm->fast_decay) ? 10 : 9);
}
...
static int32 convert_tremolo_sweep(Timid *tm, uint8 sweep)
{
    ...
    return ((tm->control_ratio * SWEEP_TUNING) << SWEEP_SHIFT) / (tm->play_mode.rate * sweep);
}
...
static int32 convert_vibrato_sweep(Timid *tm, uint8 sweep, int32 vib_control_ratio)
{
    ...
    return (int32)(FSCALE((double)(vib_control_ratio)*SWEEP_TUNING, SWEEP_SHIFT) / (double)(tm->play_mode.rate * sweep));
...
{
    return ((SINE_CYCLE_LENGTH * tm->control_ratio * rate) << RATE_SHIFT) / (TREMOLO_RATE_TUNING * tm->play_mode.rate);
}

static int32 convert_vibrato_rate(Timid *tm, uint8 rate)
{
    /* Return a suitable vibrato_control_ratio value */
    return (VIBRATO_RATE_TUNING * tm->play_mode.rate) / (rate * 2 * VIBRATO_SAMPLE_INCREMENTS);
}
...
static Instrument *load_instrument(Timid *tm, char *name, int percussion, int panning, int amp, int note_to_use, int strip_loop, int strip_envelope, int strip_tail)
{
    Instrument *ip;
    Sample     *sp;
    FILE       *fp;
...
#define READ_LONG(thing)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               \
    ...
    thing = LE_LONG(tmplong);
        ...
        else
        {
            ...
            sp->vibrato_sweep_increment = convert_vibrato_sweep(tm, tmp[15], sp->vibrato_control_ratio);
        ...
        for(j = 0; j < 6; j++)
        {
            sp->envelope_rate[j]   = convert_envelope_rate(tm, tmp[j]);
...
static int fill_bank(Timid *tm, int dr, int b)
{
    ...
    for(i = 0; i < 128; i++)
    {
        ...
        {
            /* Try SoundFont first */
            if(tm->sf_loaded && !bank->tone[i].name)
            {
                bank->tone[i].instrument = load_soundfont_instrument(tm, &tm->sf_info, tm->sf_filename, b, i);
                if(bank->tone[i].instrument)
                    continue;
            }
                ...
                if(b != 0 && tm->tonebank[0] && tm->drumset[0])
                {
                    ...
                    if(!dr)
                    {
                        if(!(tm->tonebank[0]->tone[i].instrument))
                            tm->tonebank[0]->tone[i].instrument = MAGIC_LOAD_INSTRUMENT;
                    }
                    else
                    {
                        if(!(tm->drumset[0]->tone[i].instrument))
                            tm->drumset[0]->tone[i].instrument = MAGIC_LOAD_INSTRUMENT;
                    }
                }
                bank->tone[i].instrument = 0;
                errors++;
            }
            else if(!(bank->tone[i].instrument = load_instrument(tm, bank->tone[i].name, (dr) ? 1 : 0, bank->tone[i].pan, bank->tone[i].amp, (bank->tone[i].note != -1) ? bank->tone[i].note : ((dr) ? i : -1), (bank->tone[i].strip_loop != -1) ? bank->tone[i].strip_loop : ((dr) ? 1 : -1), (bank->tone[i].strip_envelope != -1) ? bank->tone[i].strip_envelope : ((dr) ? 1 : -1), bank->tone[i].strip_tail)))
            {
                errors++;
            }
        }
...
static int32 sf_timecent_to_msec(int tc)
{
    if(tc <= -12000)
        return 1;
    if(tc >= 5000)
        return 30000;
    return (int32)(1000.0 * pow(2.0, tc / 1200.0));
}

static int32 sf_tc_to_offset(int val)
{
    ...
    if(attenuation > 1440)
        attenuation = 1440;
    return (int32)((1.0 - (double)attenuation / 1440.0) * 255.0);
}

static int32 sf_calc_envelope_rate(Timid *tm, int32 msec)
{
    int32  diff = 255;
    double rate;
    if(msec < 1)
        msec = 1;
    diff <<= (7 + 15);
    rate   = ((double)diff / tm->play_mode.rate) * tm->control_ratio * 1000.0 / msec;
    if(tm->fast_decay)
        rate *= 2;
    return (int32)rate;
}
...
Instrument *load_soundfont_instrument(Timid *tm, SFInfo *sf, const char *filename, int bank, int program)
{
    FILE         *fp;
    Instrument   *ip;
    Sample       *sp;
    ...
    int32         start, end, loop_start, loop_end, loop_mode;
    int32         attenuation, pan;
    ...
    for(i = 0; i < preset->hdr.nlayers; i++)
    {
        ...
        for(j = 0; j < inst_hdr->hdr.nlayers; j++)
        {
            ...
            sp->root_freq               = (int32)(8.176 * pow(2.0, (root_key + coarse_tune + fine_tune / 100.0) / 12.0) * 1000.0);
            sp->low_freq                = (int32)(8.176 * pow(2.0, (low_key + coarse_tune) / 12.0) * 1000.0);
            sp->high_freq               = (int32)(8.176 * pow(2.0, (high_key + coarse_tune) / 12.0) * 1000.0);

            /* Volume */
            sp->volume                  = pow(10.0, -attenuation / 200.0);

            /* Panning: SF2 uses 0=left, 500=center, 1000=right */
            sp->panning                 = (uint8)((pan + 500) * 127 / 1000);
            ...
            sp->envelope_rate[0]        = sf_calc_envelope_rate(tm, sf_timecent_to_msec(attack_tc));

            /* Stage 1: decay (peak to sustain) */
            /* SF2 sustain: 0 cB = full volume, 1000 cB = silence (centibels) */
            {
                double sus_amp         = pow(10.0, -sustain_level / 200.0);
                sp->envelope_offset[1] = (int32)(sus_amp * 255.0) << (7 + 15);
            }
            sp->envelope_rate[1]   = sf_calc_envelope_rate(tm, sf_timecent_to_msec(decay_tc)) * (sp->envelope_offset[0] - sp->envelope_offset[1]) / sp->envelope_offset[0];
            ...
            sp->envelope_rate[3]   = sf_calc_envelope_rate(tm, sf_timecent_to_msec(release_tc)) * sp->envelope_offset[2] / (255 << (7 + 15));
            ...
            {
                int32 num_samples = end - start;
                int32 num_loops   = loop_end - loop_start;
                int32 byte_offset;
                ...
                /* Allocate and read */
                sp->data         = (sample_t *)safe_malloc((num_samples + 2) * sizeof(int16));
                sp->data_alloced = 1;

                byte_offset      = sf->samplepos + start * sizeof(int16);
                fseek(fp, byte_offset, SEEK_SET);

                if(fread(sp->data, sizeof(int16), num_samples, fp) != (size_t)num_samples)
                {
                    free(sp->data);
                    sp->data = NULL;
                    continue;
                ...
                /* Peak normalize sample volume */
                {
                    ...
                    sp->volume = 32768.0 / (double)_maxamp * sp->volume;
                }
