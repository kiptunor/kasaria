...
FILE *open_file(Timid *tm, char *name, int decompress, int noise_mode)
{
    ...
    strncpy(tm->current_filename, name, 1023);
    tm->current_filename[1023] = '\0';

    if((fp = try_to_open(tm->current_filename, decompress, noise_mode)))
        return fp;
    ...
    if(name[0] != PATH_SEP)
    {
        ...
        {
            *tm->current_filename = 0;
            l                     = strlen(plp->path);
            if(l)
            {
                strcpy(tm->current_filename, plp->path);
                if(tm->current_filename[l - 1] != PATH_SEP)
                    strcat(tm->current_filename, PATH_STRING);
            }
            strcat(tm->current_filename, name);
            if((fp = try_to_open(tm->current_filename, decompress, noise_mode)))
                return fp;
