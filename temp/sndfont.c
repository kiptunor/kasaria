...
static Instrument *try_load_soundfont(SFInsts *rec, int order, int bank, int preset, int keynote);
...
static i32 to_rate(i32 diff, int timecent);
static i32 calc_rate(i32 diff, double msec);
...
static void set_rootkey(SFInfo *sf, SampleList *vp, LayerTable *tbl)
{
	...
	if(tbl->set[SF_rootKey])
		...
		vp->tune += (vp->keynote - sp->originalPitch) * 100 * (double) vp->v.scale_factor * DIV_1024;
...
/*Pseudo Reverb*/
extern i32 modify_release;

i32 get_sf_release(i32 v)
{
	i32 m = calc_rate(65535, (f64)modify_release),
		s = to_rate(65535, v);
